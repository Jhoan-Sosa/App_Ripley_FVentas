...
class FichaScreen extends ConsumerWidget {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // ✅ Botón: Enviar al comité (solo si está en evaluación)
                    _buildBotonEnviarComite(context, ref),
                  ],
                ),
              ),
            ),
  ...
  // ✅ Widget para el botón "Enviar al comité"
  // ✅ Widget para el botón "Enviar al comité"
Widget _buildBotonEnviarComite(BuildContext context, WidgetRef ref) {
  return FutureBuilder<QuerySnapshot>(
    future: FirebaseFirestore.instance
        .collection('solicitudes')
        .where('cliente_id', isEqualTo: clienteId)
        .where('estado', whereIn: ['enviado', 'en_evaluacion', 'recibido_comite'])
        .limit(1)
        .get(),
    builder: (context, snapshot) {
      ...
      final solicitudId = doc.id;
      
      // Solo mostrar si está en estado "enviado" o "en_evaluacion"
      if (estado != 'enviado' && estado != 'en_evaluacion') {
        return const SizedBox.shrink();
      }
      ...
      return Padding(
        ...
        child: ElevatedButton.icon(
          onPressed: () => _enviarAlComite(context, solicitudId),
  ...
  // ✅ Función para enviar al comité
  void _enviarAlComite(BuildContext context, String solicitudId) async {
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Enviar al comité'),
        content: const Text(
          '¿Estás seguro que deseas enviar esta solicitud al comité de evaluación?\n\n'
          'Una vez enviada, el supervisor tomará la decisión final.'
        ),
        actions: [
          TextButton(
    ...
    try {
      await FirebaseFirestore.instance
        ...
        'enviado_comite_at': DateTime.now().toIso8601String(),
      });
      
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Solicitud enviada al comité'),
            backgroundColor: Colors.green,
          ),
        );
        // Recargar la página para actualizar el estado
        context.pop();
        context.push('/ficha/$clienteId', extra: carteraId);
      }
    } catch (e) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Column(
          ...
          children: [
            ...
            ...opciones.entries.map((e) => ListTile(
                  ...
                  title: Text(e.value),
                  onTap: () async {
                    final (lat, lng) = await ubicacionVisita();
                    await ref

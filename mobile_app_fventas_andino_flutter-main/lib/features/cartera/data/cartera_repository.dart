...
class CarteraRepository {
  ...
  Future<void> guardarOrden(List<CarteraItem> items) async {
    try {
      ...
      /* el orden es local; si falla no rompe la UI */
    }
  }
  ...
  Future<ResultadoCartera> obtenerCartera({
    ...
    print('🔍 Repositorio: obteniendo cartera para $asesorId, fecha: $fecha');

    await sincronizarPendientes();
    if (await _network.isOnline) {
      try {
        print('📡 Repositorio: online, consultando remoto');
        final items = await _remote.obtenerCartera(
          asesorId: asesorId,
          fecha: fecha,
        );
        print('✅ Repositorio: ${items.length} items obtenidos');
        try {
        ...
        print('❌ Repositorio: error remoto: $e');
      }
    } else {
      print('📡 Repositorio: offline, leyendo cache');
    }
    final cache = await _local.leerCache(asesorId);
    print('📦 Repositorio: cache tiene ${cache.length} items');

...
class CarteraViewModel extends StateNotifier<CarteraState> {
  ...
  Future<void> cargar(String asesorId) async {
    ...
    try {
      print('🔍 Cargando cartera para asesor: $asesorId');
      final res = await _repo.obtenerCartera(
        asesorId: asesorId,
        fecha: DateTime.now(),
      );
      print('✅ Cartera cargada: ${res.items.length} items');
      ...
      print('❌ Error al cargar cartera: $e');
      state = state.copyWith(
        status: CarteraStatus.error,
        error: 'No se pudo cargar la cartera.',
      );
    }
  }
  ...
  Future<void> marcarVisita(
    ...
    final actualizados = state.items
        .map(
          (e) => e.id == carteraId ? e.copyWith(estadoVisita: 'visitado') : e,
        )
        .toList();
